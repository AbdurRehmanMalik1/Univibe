...
export class UserContactController {
    ...
    @UseGuards(JwtAuthGuard)
    @Post('/')
    async addContacts(@Request() req: Request,
      @Body('contacts') contacts: { contact_type: string; contact_value: string }[]
    ) {
      ...
      for (const contact of contacts) {
        ...
        if (!contact_type || !contact_value) {
          throw new BadRequestException("Each contact must have a type and value");
        }
      }
