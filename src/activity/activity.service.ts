...
export class ActivityService {
    ...
    async getActivityById(id: number){
        return await this.activityRepository.findOne({where:{activity_id:id}});
    }
    ...
    async createActivity(activity:Partial<Activity>) {
        const result = await this.activityRepository.findOne({ where: { type_name: activity.type_name } });
        if (result) {
